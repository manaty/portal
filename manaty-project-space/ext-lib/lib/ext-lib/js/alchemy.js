(function() {
      ...
      nodeMouseOut: function(n) {
        ...
        if (node._state !== "hidden") {
          ...
          if ((a.conf.nodeMouseOut != null) && typeof a.conf.nodeMouseOut === 'function') {
            return a.conf.nodeMouseOut(node);
          }
        }
      },
      copyToClipboard : function (n) {
        if (typeof a.conf.copyToClipboard === 'function') {
          a.conf.copyToClipboard(n);
        }
			},
      removeClick : function (n) {
        if (typeof a.conf.removeClick === 'function') {
          a.conf.removeClick(n);
        }
			},
  ...
  Alchemy.prototype.startGraph = function(instance) {
    ...
    return function(data) {
      ...
      d3.select("#graph-parent-group").remove();
      a.vis = d3.select(conf.divSelector).attr("style", "width:" + (conf.graphWidth()) + "px; height:" + (conf.graphHeight()) + "px; background: #FFFFFF;").append("svg").attr("xmlns", "http://www.w3.org/2000/svg").attr("xlink", "http://www.w3.org/1999/xlink").attr("pointer-events", "all").attr("style", "background: #FFFFFF;").attr("alchInst", Alchemy.prototype.instances.length - 1).on('click', a.interactions.deselectAll).call(a.interactions.zoom(conf.scaleExtent)).on("dblclick.zoom", null).append('g').attr("id", "graph-parent-group").attr("transform", "translate(" + conf.initialTranslate + ") scale(" + conf.initialScale + ")");
    ...
    return {
      ...
      styleLink: function(edge) {
        ...
        return edge.each(function(d) {
          ...
          curviness = conf.curvedEdges ? d.self._properties.largeArc : 0;
          curve = curviness / 10;
    ...
    return {
      ...
      createEdge: function(d3Edges) {
        ...
        edge.enter().append('g').attr("id", function(d) {
          ...
          return "edge " + d.edgeType+"  edge-"+d.source.self._nodeType[0]+"  edge-"+d.target.self._nodeType[0];
  ...
  DrawNode = function(instance) {
    return {
      ...
      createNode: function(node) {
        ...
        node.append('circle').attr('id', function(d) {
          return "circle-" + d.id;
           ...
           alchemy.vis.select("#tooltip-"+c.id).classed("circle-over", function (d) {
             return true;
           })
           show_node_shadow(c);
           show_tooltip(c);
        ...
        node.append('svg:foreignObject')
         ...
         .html(function (d) {
            return getIconByLabel(d.self._properties.labels[0].toLowerCase());
          })
          .on("mouseover", function (c) {
            alchemy.vis.select("#tooltip-"+c.id).classed("icon-over", function (d) {
              return true;
            })
            show_node_shadow(c);
            //node_mouseOver(c)
          ...
          .style("background-color", "transparent")
          .html(function (d) {
             return '<i class="wd-eye-minus fs2"></i>';
           })
					.on("click", function (c) {
							if (d3.event.defaultPrevented) {
			          return;
         ...
         .attr("y", "-55")
         .attr("id", function (d) {
            return "expand-btn-" + d.id;
          })
         .attr("class", "hidden node-option")
         .append("xhtml:body")
         .style("x", "-13")
         .style("y", "-55")
         .style("background-color", "transparent")
         .html(function (d) {
            return '<i class="wd-eye-plus fs2"></i>';
          })
				 .on("click", function (c) {
					 if (d3.event.defaultPrevented) {
						 return;
      ...
      nodeOptions : function (node) {
				node.selectAll(".node-option").style("display", function (c) {
					var hasEdgeOut = false;
					var nodeEdges = c.self._adjacentEdges;
					nodeEdges.forEach(function(Edge){
						if (Edge._properties.source == c.id) {
							hasEdgeOut = true;
						}
					});
  ...
  DrawNodes = function(instance) {
    return {
      ...
      createNode: function(d3Nodes) {
        ...
        drawNode.nodeOptions(node),
        drawNode.styleNode(node);
        drawNode.styleText(node);
        drawNode.setInteractions(node);

        // add tooltips group
        add_nodes_tooltips(d3Nodes);
        // add menus group
        add_nodes_menus(d3Nodes);
        // highlight nodes
        highlightNodes();
        // inactive nodes
        inactive_nodes(d3Nodes);
  ...
  EditorInteractions = (function() {
    function EditorInteractions() {
      ...
      this.removeClick = __bind(this.removeClick, this),
      this.copyToClipboard = __bind(this.copyToClipboard, this),
  ...
  Alchemy.prototype.addNewNodes = function(instance) {
    this.a.startGraph(instance);
	  this.plugins.init();
    Alchemy.prototype.instances.push(this);
  };
  ...
  text.each(function() {
    ...
    while (word = words.pop()) {
      ...
      tspan.text(line.join(" "));
        ...
        line = [word];
				++lineNumber;
        tspan = text.append("tspan").attr("x", 0).attr("y", lineNumber * lineHeight + y).attr("dy", lineNumber * lineHeight + dy).text(word);
    }
  });
}

function NodeShow(elem){
		elem.classed({
			"hidden": false
		});
}

function EdgeShow(edge){
	var Edge = alchemy.vis.select("#edge-" + edge.id +"-"+edge._index);
...
function expand(d) {
	var firstLevelNodes = [];
	var nodeID = d.id;
	alchemy.vis.select("#expand-btn-" + nodeID).classed({
		"hidden": true
	});
	alchemy.vis.select("#collapse-btn-" + nodeID).classed({
		"hidden": false
	});
  var edges = d.self._adjacentEdges;

	edges.forEach(function(targetEdge){
		if (targetEdge._properties.source == nodeID){
			firstLevelNodes.push(targetEdge._properties.target);
		}
	});

	edges.forEach(function(edge) {
		var edgeIn = false;
		if (edge._properties.source == nodeID) {
			// get node target
			var targetNode = alchemy._nodes[edge._properties.target];
			// get its relations
			var targetEdges = targetNode._adjacentEdges;

				// hide target node
				var targetNodeElement = alchemy.vis.select("#node-" + edge._properties.target);
				// hide all target node relations
				targetEdges.forEach(function(targetEdge){
					if (targetEdge._properties.source == nodeID) {
						EdgeShow(targetEdge);
						NodeShow(targetNodeElement);
					}else if(targetEdge._properties.target == edge._properties.target){
						edgeIn = true;
					}else {
						var leve2NodeElement = alchemy.vis.select("#node-" + targetEdge._properties.target);
						if (!leve2NodeElement.classed("hidden") || firstLevelNodes.indexOf(targetEdge._properties.target) != -1) {
								EdgeShow(targetEdge);
						}
...
function NodeHide(elem){
		elem.classed({
			"hidden": true
		});
}

function EdgeHide(edge){
	var Edge = alchemy.vis.select("#edge-" + edge.id +"-"+edge._index);
	Edge.classed({
		"hidden": true,
		"active": false
	...
	edges.forEach(function(edge) {
		...
		if (edge._properties.source == nodeID) {
		  ...
		  var targetEdges = targetNode._adjacentEdges;
				...
				var allEdgesHidden = true;
				targetEdges.forEach(function(target_Edge){
					var Edg = alchemy.vis.select("#edge-" + target_Edge.id +"-"+target_Edge._index);
					if (!Edg.classed("hidden")) {
							allEdgesHidden = false;
					}
				});
				if (!edgeIn || allEdgesHidden) {
					NodeHide(targetNodeElement);
				}
		}
	});
}

function toggleGraphElements(iconToggle, label) {
	var iconToggleElement = document.getElementById("toggle-icon-"+label);
	var nodeElements = alchemy.vis.selectAll("."+label);
	var nodes = nodeElements[0];

				for(var i = 0, length = nodes.length; i < length; i++) {
					var nodeID = nodes[i].id.substring(5);
					var node = alchemy._nodes[nodeID];
					var Edges = node._adjacentEdges;
						// hide all target node relations
						Edges.forEach(function(Edge){
							var sourceNode = alchemy.vis.select("#node-"+Edge._properties.source);
							var targetNode = alchemy.vis.select("#node-"+Edge._properties.target);
							var nodeElement = alchemy.vis.select("#node-"+nodeID);
							var edgeElement = alchemy.vis.select("#edge-"+Edge.id+"-"+Edge._index);
							if (hasClass(iconToggleElement, "disabled-icon")) {
								if (!nodeElement.classed("removed")) {
									...
									if (sourceNode.style("display") !="none" && targetNode.style("display") != "none" ) {
										edgeElement.style("display",'inline');
										edgeElement.classed({
											"hidden": false,
											"active": true
										});
				...
				if (hasClass(iconToggleElement, "enabled-icon")) {
					document.getElementById("toggle-icon-"+label).className="toggle-nodes-icon disabled-icon";
				}else {
					document.getElementById("toggle-icon-"+label).className="toggle-nodes-icon enabled-icon";
				}
}

function findNodeByParam(param) {
	switch(param) {
		case "first_name":
    case "last_name":
    case "birth_date":
					...
					var Edges = node._adjacentEdges;
						...
						Edges.forEach(function(Edge){
							var edgeElement = alchemy.vis.select("#edge-"+Edge.id+"-"+Edge._index);
								...
								if (Edge._properties.source == nodeID) {
									var targetNodeID = Edge._properties.target;
								}else{
									var targetNodeID = Edge._properties.source;
								}
								//remove attached node
								var targetNode = alchemy._nodes[targetNodeID];
								var targetNodeEdges = targetNode._adjacentEdges;
								var attachedNode = false;
								targetNodeEdges.forEach(function(targetEdge){
									if (targetEdge._properties.source != nodeID && targetEdge._properties.target != nodeID) {
										attachedNode = true;
									}
								});
								if (!attachedNode) {
									var targetNodeElement = alchemy.vis.select("#node-"+targetNodeID);
									// hide node
									targetNodeElement.style("display",'none');
									targetNodeElement.classed({
										"removed": true,
										"hidden": true,
										"active": false
									});
									// hide node relations
									targetNodeEdges.forEach(function(targetEdge){
										var targetEdgeElement = alchemy.vis.select("#edge-"+targetEdge.id+"-"+targetEdge._index);
											targetEdgeElement.style("display",'none');
											targetEdgeElement.classed({
												"removed": true,
												"hidden": true,
												"active": false
											});
...
function add_nodes_tooltips(d3Nodes){
  d3.select("#graph-parent-group")
  .append("g").attr('id', function() {
    return "tooltip-group";
  })
  var nodesArray = d3Nodes;
  var nodesLength = nodesArray.length;
  for (var i = 0; i < nodesLength; i++) {
      var nodeElementData = nodesArray[i];
      d3.select("#tooltip-group").append('svg:foreignObject')
      .style("display", function () {
           return "none";
      })
       .attr("width", 500)
       .attr("x", function () {
          return nodeElementData.px;
        })
       .attr("y", function () {
          return nodeElementData.py;
        })
       .attr("id", function () {
          return "tooltip-" + nodeElementData.id;
        })
       .attr("class", "node-tooltip")
       .append("xhtml:body")
       .attr("id", function () {
          return "tooltip-body-" + nodeElementData.id;
        })
       .html(function () {

         var details = "'";
         Object.keys(nodeElementData.self._properties).map(function (key) {
           details += key+": "+nodeElementData.self._properties[key]+"&";
         });
         details +="'";
         var allInfo = {};

         var properties = '<div class="node-header">'+getIconByLabel(nodeElementData.self._properties.labels[0].toLowerCase())+'<span class="node-title">'+nodeElementData.self._properties.labels[0]+'</span></div>';
         properties += '<button class="btn tooltip-copy-btn" onClick="pre_copy('+details+')"><i class="wd-copy fs2"></i></button>'
         properties += '<span class="copy-status text-success hidden">Copied</span>'
            Object.keys(nodeElementData.self._properties).map(function (key) {

              if (nodeElementData.self._properties.labels[0].toLowerCase() == "website" && key.toLowerCase() == "domainname") {
                //if website
                var domaineName = nodeElementData.self._properties[key];
                if (domaineName) {
                  if (!/^((http|https|ftp):\/\/)/.test(domaineName)) {
                      domaineName = "http://" + domaineName;
                   }
                }
                properties += '<div><span class="node-prop">'+key+'</span>:<a href='+domaineName+' target="_BLANK" class="prop-value">'+nodeElementData.self._properties[key]+'</a></div>'
              } else {
                properties += '<div><span class="node-prop">'+key+'</span>:<span class="prop-value">'+nodeElementData.self._properties[key]+'</span></div>'
              }
...
function add_nodes_menus(d3Nodes){
  d3.select("#graph-parent-group")
  .append("g").attr('id', function() {
    return "menu-group";
  })
  ...
  for (var i = 0; i < nodesLength; i++) {
      var nodeElementData = nodesArray[i];
      var foreignObject = d3.select("#menu-group").append('svg:foreignObject')
        .style("display", function (nodeElementData) {
           return "none";
         })
       .attr("width", 300)
       .attr("height", 300)
       ...
       .attr("id", function () {
          return "menu-" + nodeElementData.id;
        })
       .attr("class", "node-menu");
       var body = foreignObject.append("xhtml:body")
       var ul = body.append("ul");
       ul.html(function () {
         return '<li onclick="pre_removeClick('+nodeElementData.id+')"><span class="wd-trash3 fs2"></span>Delete</li>';
       });

       ul.append("li")
       .on("click", function(){
         //return alchemy.interactions.removeClick(nodeElementData);
       })
       .html(function () {
         return '<span class="wd-link fs2"></span>Link</li>';
       });
       ...
       .html(function () {
         return '<span class="wd-plus fs2"></span>Add a node</li>';
       });
       ...
       .html(function () {
         return '<span class="wd-refresh fs2"></span>Refresh</li>';
       });

  }
}
function show_node_shadow(node){
  alchemy.vis.select("#defs-"+node.id).style("display", function (d) {
    return "block";
  })
  alchemy.vis.select("#circle-"+node.id).style("filter", function(d) {
    return "url(#filter-"+node.id+")";
  })
}

function show_tooltip(node){
  if (!alchemy.vis.select("#menu-"+node.id).classed("visible")) {
    alchemy.vis.select("#tooltip-"+node.id).classed("visible", function (d) {
      return true;
    })


    setTimeout(function(){
        if (alchemy.vis.select("#tooltip-"+node.id).classed("visible")) {
          if (alchemy.vis.select("#tooltip-"+node.id).classed("icon-over") || alchemy.vis.select("#tooltip-"+node.id).classed("circle-over")) {

            alchemy.vis.select("#tooltip-"+node.id).style("display", function (d) {
              return "block";
            })
            // calculate tooltips dimensions
            var tooltip_divs_heights = 0;
            var ancestor = document.getElementById("tooltip-body-" + node.id),
            descendents = ancestor.getElementsByTagName('div');
            for (i = 0; i < descendents.length; ++i) {
                des = descendents[i];
                tooltip_divs_heights += des.offsetHeight;
            };
            // change tooltip position
              alchemy.vis.select("#tooltip-"+node.id).attr("x", function (d) {
              ...
              alchemy.vis.select("#tooltip-"+node.id).attr("y", function (d) {
                var isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
                var isChrome = /Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor);
                if(isFirefox){
                  return node.y - 20;
            ...
            .attr("height", function(){
              return tooltip_divs_heights + 160;
            })
            .on("mouseover", function (c) {
              show_node_shadow(node);
            })
            .on("mouseout", function (c) {
              hide_node_shadow(node);
              hide_tooltip(node);
            })
            .on("click", function (c) {
              //alchemy.interactions.nodeClick(node)
            });
            // set tooltip body height
            if(navigator.userAgent.toLowerCase().indexOf('firefox') > -1){
              // for Firefox
              alchemy.vis.select("#tooltip-body-"+node.id).style("height", function (d) {
                return "auto"
              })
            }
          }
...
function hide_node_shadow(node){
  alchemy.vis.select("#defs-"+node.id).style("display", function (d) {
    return "none";
  })
  alchemy.vis.select("#circle-"+node.id).style("filter", function(d) {
...
function node_click(nodeID){
  //toggle node selection
  alchemy.vis.select("#node-"+nodeID).classed("selected-node", function (d){
     if (alchemy.vis.select("#node-"+nodeID).classed("selected-node")) {
       return false
  	 } else {
  	 	 return true
  	 }
...
function node_rightClick(node){
  //hide tooltips
  closeAllTooltips();
  // close menus
  closeAllMenus();
  //set node selection
  alchemy.vis.select("#node-"+node.id).classed("selected-node", function (d){
  	return true
  })
  // change menu position
    alchemy.vis.select("#menu-"+node.id).attr("x", function (d) {
      return node.x - 10;
    })
    alchemy.vis.select("#menu-"+node.id).attr("y", function (d) {
      var isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
      if(isFirefox){
        return node.y + 20;
      }else{
  ...
  // show menu
  alchemy.vis.select("#menu-"+node.id).classed("visible", function (d) {
    return true;
  })
}

function pre_copy(data){
  alchemy.vis.selectAll(".copy-status").classed("hidden", function (d) {
    return false;
  })
  setTimeout(() => {
    alchemy.vis.selectAll(".copy-status").classed("hidden", function (d) {
  ...
  alchemy.interactions.copyToClipboard(data);
}

function pre_removeClick(nodeID){
  closeAllMenus();
  closeAllTooltips();
  var data_array = [nodeID];
  alchemy.interactions.removeClick(data_array);
}

function closeAllMenus(){
  // close menus
  alchemy.vis.selectAll(".node-menu").classed("visible", function (d) {
    return false;
  })
}
...
function closeAllTooltips(){
  //hide tooltips
  alchemy.vis.select(".node-tooltip").classed("visible", function (d) {
    return false;
  })
  alchemy.vis.selectAll(".node-tooltip").style("display", function (d) {
    return "none";
  })
}

function deselectAllNodes(){
  alchemy.vis.selectAll(".node").classed("selected-node", function (d) {
    return false;
  })
}

function highlightNodes(){
  var search_nodes = JSON.parse(localStorage.getItem('vpp_search_nodes'));
  if (search_nodes!=null) {
    for (var i = 0; i < search_nodes.length; i++) {
       d3.select("#circle-"+search_nodes[i]).classed("highlighted-node", function (d) {
         return true;
       })
     }
  }
...
function inactive_nodes(d3Nodes){
  var nodesArray = d3Nodes;
  var nodesLength = nodesArray.length;
  for (var i = 0; i < nodesLength; i++) {
      var nodeElementData = nodesArray[i];
      if (nodeElementData.self._properties.labels[0].toLowerCase() == "person") {
        if (nodeElementData.self._properties.deadDate && nodeElementData.self._properties.deadDate != null) {
            d3.select("#node-"+nodeElementData.id).classed("inactive-node", true);
        }
      }else if (nodeElementData.self._properties.labels[0].toLowerCase() == "company") {
        if (nodeElementData.self._properties.status == "inactif" || nodeElementData.self._properties.closureDate) {
            d3.select("#node-"+nodeElementData.id).classed("inactive-node", true);
        }
      }
  }
...
function inactive_edges(edges){
  var edgesArray = edges;
  var edgesLength = edgesArray.length;
  for (var i = 0; i < edgesLength; i++) {
    var edgeElementData = edgesArray[i];
      //if (edgeElementData.self._properties.until && edgeElementData.self._properties.until != null) {
          d3.select("#edge-"+edgeElementData.id+"-"+edgeElementData.pos).classed("inactive-edge", true);
      //}
  }
}
