...
public class UserApi extends BaseApi {
    ...
    public void update(UserDto postData) throws MeveoApiException, BusinessException {
        ...
        if (!validationService.validateUniqueField("User", "email", user.getId(), user.getEmail())) {
            throw new BusinessApiException("email already existed");
        }
        ...
        if (isUsersManager && CollectionUtils.isNotEmpty(roles)) {
            user.setRoles(roles);
        }
        if (isUsersManager && CollectionUtils.isNotEmpty(securedEntities)) {
            user.setSecuredEntities(securedEntities);
        }
