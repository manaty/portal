...
public class NativePersistenceService extends BaseService {
     ...
     * @return The uuid of the record if it was found or null if it was not
     */
    public List<Map<String, Object>> findValueByLikeStringValues(String tableName, List<String> selectFields, Map<String, String> queryValues) {
        StringBuilder selectQuery = new StringBuilder("SELECT ");
        ...
        QueryBuilder queryBuilder = new QueryBuilder(selectQuery + " FROM " + tableName + " a ", "a");

        queryValues.forEach((key, value) -> {
            queryBuilder.like(key, value, QueryBuilder.QueryLikeStyleEnum.MATCH_ANYWHERE,false);
        });
        SQLQuery query = queryBuilder.getNativeQuery(getEntityManager(), true);
        try {
            return query.list();
